...
#' @export scs.stations
scs.stations <- function(n, longitude, latitude, hole = NULL, alternates = 0, grids = FALSE){
  ...
  if (is.list(grids)){
    ...
    while (!flag){
      ...
      if (flag){
        ...
        #temp$grid <- deg2grid(temp$x, temp$y, ref.x = bb[1], ref.y = bb[2], delta.x = d, delta.y = d)
      }
    }
  ...
  if (alternates > 0){
    for (j in 1:alternates){
      temp <- scs.stations(longitude = longitude, latitude = latitude, hole = hole, grids = grids)
  ...
  if (include.grids){
    if (complete){
      ...
      # Assign grid identifier to each grid using mid-point:
      #grid.names <- grid2str(apply(xx, 1, mean), apply(yy, 1, mean), ref.x = bb[1], ref.y = bb[2], delta.x = d, delta.y = d)
...
#' @export nss.stations
nss.stations <- function(n, alternates = 0, stations, type){
  # Check 'n' argument:
